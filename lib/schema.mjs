...
const stats = z.object({ attack: fact(integer), interval: fact(z.number().nonnegative()), target: fact(text) }).strict().refine(s => s.interval.value !== 0 || s.target.value === '攻撃不可', '攻撃間隔0は攻撃不可の表示を確認した場合のみ登録できます');
...
export const minionSchema = z.object({
  ...
  icon: id.optional(), portrait: id,
  evaluation: z.object({ rank: z.enum(['SS','S','A','B','C','未評価','対象外']), scope: text, source: text }).strict(),
  availability: z.object({ acquisition: fact(z.enum(['obtainable','unobtainable'])), ranked: fact(z.boolean()) }).strict().optional(),
...
export const sourcesSchema = z.record(text, z.object({ kind: z.enum(['screenshot','user']), description: text,
  ...
  screen: z.enum(['detail','pool-overview','pool-list','bond-description']).optional(),
  selectedStar: z.number().int().min(1).max(4).optional(),
...
const bond = z.object({
  icon: fact(id).optional(),
  name: fact(text),
  tiers: fact(z.array(z.object({
    threshold: z.number().int().positive(), text,
    amount: z.number().positive(), unit: z.enum(['points', 'percent', 'coins'])
  }).strict()).min(1).refine(v => v.every((t,i) => i === 0 || t.threshold > v[i-1].threshold), '絆の段階は重複なく昇順で記録してください')),
  tierApplication: fact(z.enum(['replace', 'cumulative'])),
  mechanics: fact(text)
}).strict();
const knownOrUnknown = value => fact(value).refine(f => f.status !== 'absent', '対象項目の未確認はunknownを使用してください');
const rumbleMinion = z.object({
  availability: knownOrUnknown(z.boolean()),
  star4: knownOrUnknown(z.object({
    ability,
    currentLevel: knownOrUnknown(level),
    guaranteedLevel: knownOrUnknown(level),
    conditions: text
  }).strict()).optional()
}).strict().superRefine((v,c) => {
  if (v.availability.value === true && !v.star4) c.addIssue({code:'custom',message:'登場するミニオンには星4能力または未確認の理由が必要です'});
  if (v.availability.value !== true && v.star4) c.addIssue({code:'custom',message:'登場未確認・非登場のミニオンに星4能力は登録できません'});
});
export const rumbleSchema = z.object({
  schemaVersion: z.literal(1), mode: z.literal('rumble-chess'), updated: text, note: text,
  rules: z.object({
    sharedPerformance: knownOrUnknown(text),
    minimumGuaranteedLevel: knownOrUnknown(level),
    bondPointsByStar: knownOrUnknown(z.object({'1':integer,'2':integer,'3':integer,'4':integer}).strict())
  }).strict(),
  memberIds: z.record(text, id),
  minions: z.record(id, rumbleMinion),
  decks: z.array(z.object({id, name: fact(text), members: fact(z.array(text).min(1)), bond}).strict()).refine(v=>new Set(v.map(d=>d.id)).size===v.length,'デッキIDの重複'),
